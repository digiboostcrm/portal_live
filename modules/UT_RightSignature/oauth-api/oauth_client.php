...
{/metadocument}
*/
	var $configuration_file = 'modules/UT_RightSignature/oauth-api/oauth_configuration.json';
...
*/
	...
	var $oauth_user_agent = 'PHP-OAuth-API - Customized by Urdhva Tech';
...
*/
	Function Initialize()
	{
		...
		switch($this->server)
		{
            ...
            case 'RightSignature':
                $this->oauth_version = '2.0';
                $this->dialog_url = 'https://api.rightsignature.com/oauth/authorize?response_type=code&client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI}&scope={SCOPE}&state={STATE}';
                $this->access_token_url = 'https://api.rightsignature.com/oauth/token';
                $this->authorization_header = false;
                break;
			...
			default:
				...
				if(!IsSet($oauth_server->servers->{$this->server}))
					return($this->SetError($this->server.' is not yet a supported type of OAuth server. Please send a request in this class support forum (preferred) http://www.phpclasses.org/oauth-api , or if it is a security or private matter, to request adding built-in support to this type of OAuth server.'));
