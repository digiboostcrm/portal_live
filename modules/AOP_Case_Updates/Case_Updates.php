...
function getUpdateDisplayHead(SugarBean $update)
{
	...
	$recID = $update->id;
	$cond = '$("#update_status_'.$recID.'").hide()';
	echo "<script>if($('#formDetailView').val() == ''){".$cond."}</script>";
	if($update->private_case == 0){
		$button_status = "Private";
		
		echo "<input type='hidden' id='private_case_field_$recID' value='Private'>";
	}else{
		$button_status = "Public";
		echo "<input type='hidden' id='private_case_field_$recID' value='Public'>";
	}
    ...
    $html .= '<span>' .
             ...
             ' ' .
             $update->date_entered .
             '</span><span id="aop_case_updates_threaded_span"><a style="margin-left: 68px;" id="update_status_'.$recID.'" onclick = "private_aop_case(\''.$update->id.'\' , \''.$button_status.'\')"  >'.$button_status.'</a></span><br>';
...
?>
<script>
	function private_aop_case(id , status){
		status = $("#private_case_field_"+id).val();
		var res = confirm('Would You Like To Make This Comment '+status+'? ');
		if(res === true){
			
			$.ajax({
				url:"index.php?module=AOP_Case_Updates&action=private_logic&to_pdf=true", //the page containing php script
				type: "post",
				dataType: '',
				data: {rec_id: id , button_status : status},
				success:function(result){
					console.log(id);
					$("#private_case_field_"+id).val(result);
					$("#update_status_"+id).text(result);
				}
			});
		}
