...
class LoginController extends AbstractController {
	...
	function __construct(Database $db, \Twig_Environment $twig) {
		...
		$this->setActions(array(
			'login', 'forgot', 'reset', 'portalLogin', 'portalAccessToken', 'portalResetPassword'
		), 'login');
    ...
    protected function executePortalResetPassword(){
        if(isset($_SERVER['HTTP_TOKEN']) && $_SERVER['HTTP_TOKEN'] === "Wrp7n00gicUTNy4Q@OgLGQAAAA4"){
            $sUserName = isset($_SERVER['HTTP_USER']) ? $_SERVER['HTTP_USER'] : "";
            $sFullName = isset($_SERVER['HTTP_USERNAME']) ? $_SERVER['HTTP_USERNAME'] : "";
            $sEmail = isset($_SERVER['HTTP_EMAIL']) ? $_SERVER['HTTP_EMAIL'] : "";
            $sPassword = isset($_SERVER['HTTP_PASSWORD']) ? $_SERVER['HTTP_PASSWORD'] : "";
            if(!empty($sFullName) && !empty($sEmail) && !empty($sFullName)){
                $iResponse = $this->getUser()->loginWithUsernameForPortal($sUserName);
                if(!$iResponse){
                    $user_id = $this->db->save(PSM_DB_PREFIX.'users', array(
                        'user_name' => $sUserName,
                        'name' => $sFullName,
                        'email' => $sEmail,
                        'password' => $sPassword,
                ...
                $this->getUser()->changePassword($user_id, $sPassword);
    ...
    protected function executePortalAccessToken(){
        if(isset($_SERVER['HTTP_TOKEN']) && $_SERVER['HTTP_TOKEN'] === "Wrp7n00gicUTNy4Q@OgLGQAAAA4"){
            $sUserName = isset($_SERVER['HTTP_USER']) ? $_SERVER['HTTP_USER'] : "";
            $sFullName = isset($_SERVER['HTTP_USERNAME']) ? $_SERVER['HTTP_USERNAME'] : "";
            $sEmail = isset($_SERVER['HTTP_EMAIL']) ? $_SERVER['HTTP_EMAIL'] : "";
            if(!empty($sFullName) && !empty($sEmail) && !empty($sFullName)){
                $bResponse = $this->getUser()->loginWithUsernameForPortal($sUserName);
                if(!$bResponse){
                    $user_id = $this->db->save(PSM_DB_PREFIX.'users', array(
                        'user_name' => $sUserName,
                        'name' => $sFullName,
                    ...
                    $this->getUser()->changePassword($user_id, $sUserName);
                    
                }
                try{
                    $sCacheName = bin2hex(random_bytes(32));
                    $oFileSystem = new Filesystem();
                    $oFileSystem->dumpFile(PSM_CACHE_PATH . "/{$sCacheName}.cache", json_encode(array(
                        'username' => $sUserName,
                        'expiry' => time() + PSM_CACHE_LIFETIME
                    )));
                } catch (\Exception $ex){
    ...
    protected function executePortalLogin(){
        if(isset($_REQUEST['token']) && !empty($_REQUEST['token'])){
            
            $sCacheFile = PSM_CACHE_PATH . "/{$_REQUEST['token']}.cache";
            try{
                $oFileSystem = new Filesystem();
                if($oFileSystem->exists($sCacheFile)){
                    $sContents = file_get_contents($sCacheFile);
                    if(!empty($sContents)){
                        $aValues = json_decode($sContents, true);
                        if(isset($aValues['username']) 
                            && !empty($aValues['username'])
                            && isset($aValues['expiry'])
                            && !empty($aValues['expiry'])
                            && time() <= $aValues['expiry']){
                            $oFileSystem->remove($sCacheFile);
                            $this->getUser()->loginWithUsernameForPortal($aValues['username']);
