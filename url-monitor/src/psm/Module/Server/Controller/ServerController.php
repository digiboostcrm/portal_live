		...
		switch($this->server_id) {
			...
			default:
				...
				if(empty($edit_server) || 
                                        ($this->getUser()->getUserLevel() == PSM_USER_USER && !in_array($this->getUser()->getUserId(), $user_idc_selected))
                                    ) {
		...
		if(!empty($edit_server)) {
			...
			$tpl_data = array_merge($tpl_data, array(
				...
				'edit_pushover_selected_' . $edit_server['pushover'] => 'selected="selected"',
                                'is_admin' => $this->getUser()->getUserLevel() == PSM_USER_ADMIN
			));
		}
		...
		if (empty($_POST)) {
         ...
         if ( !empty( $_POST['website_password'] ))          {
             ...
             if ($this->server_id > 0) {
                 ...
                 if($this->getUser()->getUserLevel() == PSM_USER_USER && !in_array($this->getUser()->getUserId(), $user_idc_selected)){
                    return $this->executeIndex();
                 }
