...
*/
    function display() {
		...
		$queueID = $this->bean->fetched_row['custom_user_type'];
		$sql = 'SELECT id, name FROM acl_roles WHERE deleted = 0';
		$res = $db->query($sql);
		$selectedAdmin = "";
		$selectedReg = "";
		if($queueID == 'Administrator'){
			$selectedAdmin = "selected";
		}
		else if($queueID == 'regular_user'){
			$selectedReg = "selected";
		}
		
		$options = "<option $selectedReg value='regular_user'>Regular User</option>";
		$options .= "<option $selectedAdmin value='Administrator'>Administrator</option>";
		...
		$dropDown = "<select name='custom_user_type' id='custom_user_type'>$options</select>";
		//echo $options;die;
		
		$this->ss->assign('Custom_USER_TYPE_DROPDOWN', $dropDown);
...
EOD
		...
		print_r($current_user->is_admin);
		/*
		if( $queueID != 'Administrator' ){			
			echo '<script> $(\'#custom_user_type\').attr("disabled", true);  </script>';
		}*/
