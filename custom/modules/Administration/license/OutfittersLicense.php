...
if(!class_exists('OutfittersLicense')) 
{
    class OutfittersLicense
    {
        ...
        public static function isValid($thisModule=null,$user_id=null,$force_validation=false)
        {
            ...
            require('custom/modules/'.$thisModule.'/license/config.php');
            require_once 'custom/modules/'.$thisModule.'/license/tpls/license.tpl';
        ...
        public static function validate()
        {
            ...
            $administration->retrieveSettings('PortalPlugin');

            
            $currDate = date("Y/m/d");
            if ($validated['success'] == 1) {
                $administration->saveSetting("PortalPlugin", "LastValidation", 1);
                $administration->saveSetting("PortalPlugin", "LastValidationDate", $currDate);
                $administration->saveSetting("PortalPlugin", "LastValidationMsg", '');
            } else {
                $administration->saveSetting("PortalPlugin", "PortalInstance", '');
                $administration->saveSetting("PortalPlugin", "PortalInstance_url", '');
                $administration->saveSetting("PortalPlugin", "LastValidation", 0);
                $administration->saveSetting("PortalPlugin", "LastValidationDate", $currDate);
                $administration->saveSetting("PortalPlugin", "LastValidationMsg", !empty($validated['success']) ? $validated['success'] : 'There seems some error while validating your license for Customer Survey module. Please try again later.');
            }
...
if (!class_exists('SugarOutfitters_API'))
{
    class SugarOutfitters_API
    {
        private function get_default_payload($module,$custom_data = array())
        {
            ...
            //load license validation config
            require('custom/modules/'.$module.'/license/config.php');
