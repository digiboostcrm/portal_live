	...
	public function display()
    {
        ...
        foreach($this->defs as $module => $vdef)
        {
            if(!isset($beanList[$module]))
            {
				...
				//$GLOBALS['log']->fatal(print_r($beanList[$module] , 1));
		...
		echo '<script type="text/javascript" src="custom/modules/Leads/js/leadConvert.js"></script>';
		global $current_user;
		echo '<script type="text/javascript">
			$("#Opportunitiesuser_opp_owner_c").val("'.$current_user->name.'"); 
			$("#Opportunitiesuser_id_c").val("'.$current_user->id.'"); 
		</script>';
    }
